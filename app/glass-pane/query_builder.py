...
class CanonicalQueryBuilder:
    ...
    def build_list_query(self, params: LogQueryParams) -> Dict[str, Any]:
        """Build query for listing logs with filters."""
        bq_params = []
        where_clauses = []
        ...
        # Optional service filter (partial match)
        if params.service:
        ...
        # Optional search filter (searches display_message and json_payload)
        if params.search:
            where_clauses.append(
    ...
    def build_count_by_severity_query(self, hours: int = 24) -> Dict[str, Any]:
        """Build query to count logs by severity."""
        sql = f"""
            SELECT
                severity,
                COUNT(*) as count
    ...
    def build_count_by_service_query(self, hours: int = 24) -> Dict[str, Any]:
        """Build query to count logs by service."""
        sql = f"""
            SELECT
                service_name,
                COUNT(*) as count,
    ...
    def build_source_table_stats_query(self, hours: int = 24) -> Dict[str, Any]:
        """Build query to get stats by source table."""
        sql = f"""
            SELECT
                source_table,
                COUNT(*) as count
